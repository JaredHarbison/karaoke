...
policy, duration, and theme review state are established before a Performance is
admitted. Performance owns theme review, rejection, and post-window release
transitions. Duration is snapshotted on the Performance with its source
(`provider` or `average`) for runtime cutoff decisions.
...
adapter, and physical table renames remain deferred.
