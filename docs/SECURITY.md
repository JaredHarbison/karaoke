...
contextual `VenueMembership` records; the legacy `VenueAdmin` table and global
venue-role column have been removed. Application-wide staff access is a
separate `platform_admin` capability and must not be used as a venue-context
authorization substitute. Event-level authorization is future work. Run the
configured security checks and report their actual results.
