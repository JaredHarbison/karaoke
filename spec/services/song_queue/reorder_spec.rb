...
RSpec.describe SongQueue::Reorder, type: :service do
  it 'keeps host reordering scoped to the song event' do
    ...
    first = create(:performance, event: event, venue: event.venue, updated_at: 2.hours.ago)
    target = create(:performance, event: event, venue: event.venue, updated_at: 1.hour.ago)
    other = create(:performance, event: other_event, venue: other_event.venue, updated_at: 3.hours.ago)

    host = event.venue.owner
    described_class.pause!(target, 1, actor: host)

    expect(Performance.unscoped.where(event_id: event.id).order(:updated_at).pluck(:id)).to eq([first.id, target.id])
  ...
  it 'locks the event while reordering an event queue' do
    event = create(:event)
    song = create(:performance, event: event, venue: event.venue)
  ...
  it 'keeps simultaneous event reorders consistent', use_transactional_fixtures: false do
    ...
    first = create(:performance, event: event, venue: venue, updated_at: 2.hours.ago)
    second = create(:performance, event: event, venue: venue, updated_at: 1.hour.ago)
    host_id = venue.owner_id

    run_concurrently([first.id, second.id]) do |song_id|
      described_class.pause!(Performance.find(song_id), 1, actor: User.find(host_id))
    end

    queue_ids = Performance.unscoped.where(event_id: event.id).order(:updated_at, :id).pluck(:id)
