...
RSpec.describe SongQueue::FairOrder, type: :service do
  ...
  it 'uses user identity rather than display name when available' do
    event = create(:event)
    user = create(:user)
    create(:song, :finished, event: event, venue: event.venue, user: user, performer: 'Stage Name')
    queued = create(:song, event: event, venue: event.venue, user: user,
                           performer: 'Different Label',
                           updated_at: 2.hours.ago)
    new_singer = create(:song, event: event, venue: event.venue,
                               performer: 'New Singer',
                               updated_at: 1.hour.ago)

    result = described_class.new(Song.where(id: [queued.id, new_singer.id]), event: event).call
  ...
  it 'does not count skipped songs as completed turns' do
    event = create(:event)
    create(:song, :finished, :skipped, event: event, venue: event.venue, performer: 'Skipped Singer')
    skipped_singer = create(:song, event: event, venue: event.venue,
                                   performer: 'Skipped Singer', updated_at: 2.hours.ago)
    new_singer = create(:song, event: event, venue: event.venue, performer: 'New Singer', updated_at: 1.hour.ago)

    result = described_class.new(Song.where(id: [skipped_singer.id, new_singer.id]), event: event).call
